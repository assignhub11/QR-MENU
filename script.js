...
const menuItems = [
    ...
    },
    {
        id: 7,
        name: "Biryani",
        description: "Fragrant rice cooked with herbs, spices and your choice of protein",
        price: 280,
...
function createQRCodeSection() {
    ...
    qrCodeSection.innerHTML = `
        ...
        <p>Scan with any UPI app to complete the payment</p>
    `;
    
    // Find the place to insert the QR code section
    const cartContent = document.querySelector('.cart-content');
    const cartCheckout = document.querySelector('.cart-checkout');
    
    // Insert before the checkout section
    cartContent.insertBefore(qrCodeSection, cartCheckout);
...
function generateQRCode(amount) {
    ...
    // Create UPI payment string (this is a sample format)
    // Format: upi://pay?pa=merchant@upi&pn=Restaurant&am=amount&cu=INR&tn=Food Order
    const upiString = `upi://pay?pa=restaurant@upi&pn=FoodOrder&am=${amount}&cu=INR&tn=FoodOrder${Date.now()}`;
    ...
    if (typeof QRCode !== 'undefined') {
        ...
        qrCodeDisplay.innerHTML = `
            <div style="width: 200px; height: 200px; margin: 0 auto; background: #f5f5f5; display: flex; align-items: center; justify-content: center; border: 1px dashed #ccc;">
                <span>Unable to load QR Code</span>
            </div>
            <p style="margin-top: 10px;">Amount to pay: ₹${amount}</p>
        `;
    }
}
...
function setupEventListeners() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        if (searchTerm.length > 0) {
            ...
            categories.forEach(category => {
                ...
                const categoryItemsContainer = document.getElementById(`search-${category}-items`);
                categoryItems.forEach(item => {
                    categoryItemsContainer.appendChild(createMenuItemElement(item));
                });
            });
    ...
    paymentMethods.forEach(method => {
        method.addEventListener('click', function() {
            ...
            // Show QR code only when UPI is selected
            const qrCodeSection = document.getElementById('qrCodeSection');
            if (qrCodeSection) {
                if (this.dataset.method === 'upi') {
                    qrCodeSection.style.display = 'block';
                    // Calculate the total amount for the QR code
...
function updateCartUI() {
    ...
    if (cart.length === 0) {
        ...
        // If UPI payment method is selected, update the QR code
        const activePaymentMethod = document.querySelector('.payment-method.active');
        if (activePaymentMethod && activePaymentMethod.dataset.method === 'upi') {
            const qrCodeSection = document.getElementById('qrCodeSection');
            if (qrCodeSection) {
                qrCodeSection.style.display = 'block';
                const total = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    ...
    // No tax calculation (GST removed as requested)
    const total = subtotal;
    
    cartSubtotal.textContent = `₹${subtotal.toFixed(2)}`;
    // Remove reference to cartTax as we're removing GST
    cartTotal.textContent = `₹${total.toFixed(2)}`;
    
    // Update the cart summary section to remove the tax row
    const cartSummary = document.querySelector('.cart-summary');
    if (cartSummary) {
        const taxRow = cartSummary.querySelector('.tax-row');
        if (taxRow) {
...
// Add this code to the bottom of your existing script.js file to integrate the bill page

// Modify the checkout button handler to store payment method and redirect to bill page
function setupBillIntegration() {
    // Update checkout button to redirect to bill page
    const originalCheckoutBtnHandler = checkoutBtn.onclick;
    checkoutBtn.onclick = function() {
        ...
        localStorage.setItem('selectedPaymentMethod', getPaymentMethodLabel(selectedPayment));
...
function generateBill() {
    ...
    return {
        ...
        paymentMethod: localStorage.getItem('selectedPaymentMethod') || 'Cash Payment'
    };
}
            ...
            <table class="bill-items">
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="text-right">₹${billData.subtotal.toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td colspan="3" class="text-right"><strong>Total:</strong></td>
                        <td class="text-right total-amount">₹${billData.total.toFixed(2)}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
